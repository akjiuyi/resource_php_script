<?php
                ...
                foreach ($origin_videos as $video) {
                    try {
                        ...
                        if(is_file($source_video)){
                            ...
                            $data = [];
                            $data['state'] = 11;
                            $data['m3u8_key'] = $watermark_relative_path;

                            $res = update_video_process_data($conn,$video['id'],$data);
                            if ($res) {
                                echo "watermark of video file process success\n";
                            } else {
                                echo "watermark of video file process failed\n";
                            }
    ...
    function get_origin_video($conn){
        $sql = "select id,m3u8_key from mzfk_video where process_state = 7 and m3u8_key <> '' and create_time>1629129600 and id in (10093)";
