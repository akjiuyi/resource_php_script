<?php
    ...
    function interactiveMsg($conn)
    {
        ...
        for ($i=1; $i<=$num; $i++){
            ...
            $before_one_hour = date('Y-m-d H:i:s',strtotime("-1 hour"));
            ...
            foreach ($actor_like as $v){
                ...
                $before_one_hour_timestamp = strtotime($before_one_hour);
                //$con = "actor_id = {$v['actor_id']} and create_time>$yestoday_timestamp";
                $con = "actor_id = {$v['actor_id']} and create_time>$before_one_hour_timestamp";
                ...
                foreach ($videos as $u){
                    ...
                    $create_time = time();
                    $data = "(title,type,member_id,content,actor_id,jump_url,create_time) values ('互动提示',1,{$v['member_id']},'$desc',{$v['actor_id']},'$jump_url',$create_time)";
