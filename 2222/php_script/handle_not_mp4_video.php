<?php
    ...
    function handle_not_mp4_video($conn){
                ...
                foreach ($origin_videos as $video) {
                    try {
                        ...
                        if(is_file($source_video)){
                            preg_match("/(.+)\.(mov|wmv|flv|avi|mkv)$/i", $source_video, $matches);
                            if(isset($matches[2])){
                                preg_match("/(.+)\.(mov|wmv|flv|avi|mkv)$/i", $video['m3u8_key'], $matches1);
                                //echo "ffmpeg -i {$source_video} -c:v libx264 -c:a aac  {$matches[1]}.mp4\n";
                                //echo "{$matches1[1]}.mp4\n";
                                exec("ffmpeg -i {$source_video} -c:v libx264  -c:a aac  {$matches[1]}.mp4");

                                //存入压片完毕状态  11
                                $data = [];
                                $data['m3u8_key'] = "{$matches1[1]}.mp4";
                                $data['process_state'] = 7;

                                $res = update_video_m3u8_key($conn,$video['id'],$data);
                                if ($res) {
                                    echo "handling of video:id={$video['id']} file success\n";
                                } else {
                                    echo "handling of video:id={$video['id']} file failed\n";
                                }
                            }else{
                                echo " video:id={$video['id']} file match failed\n";
                            }
    ...
    //更新视频路径
    function update_video_m3u8_key($conn,$id,$data){
        $sql = "UPDATE mzfk_video SET m3u8_key='{$data['m3u8_key']}',process_state={$data['process_state']}  WHERE id=$id";
        echo "{$sql}\n";
    ...
    function get_origin_video($conn){
        ...
        $sql = "select id,m3u8_key from mzfk_video where process_state=100 and m3u8_key <> '' and id>3755 order by id asc limit 0,2000";
